...
export async function GET(request: NextRequest) {
  ...
  if (type === 'industry-trends') {
    const byIndustry = startupData.reduce(
      (acc, s) => {
  ...
  if (type === 'stage-analysis') {
    const byStage = startupData.reduce(
      (acc, s) => {
  ...
  const totalRevenue = startupData.reduce((sum, s) => sum + (s.revenue || 0), 0);
  const totalMrr = startupData.reduce((sum, s) => sum + (s.mrr || 0), 0);
  const avgRevenue = Math.round(totalRevenue / startupData.length);
  const avgMrr = Math.round(totalMrr / startupData.length);
  ...
  return NextResponse.json({
    ...
    avgRevenue,
    avgMrr,
    topByRevenue: startupData.sort((a, b) => (b.revenue || 0) - (a.revenue || 0)).slice(0, 5),
    topByMrr: startupData.sort((a, b) => (b.mrr || 0) - (a.mrr || 0)).slice(0, 5),
  });
}
